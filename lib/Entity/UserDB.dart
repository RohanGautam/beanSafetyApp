...
class UserDB {
  ...
  Future updateUserData(double latitude, double longitude, bool alerter,
      bool alerted, bool responder, String alertType, int alertLevel) async {
  ...
  Future updateUserDataOnRegister(
      double latitude,
      double longitude,
      bool alerter,
      bool alerted,
  ...
  List<UserData> _userDataFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.documents.map((doc) {
      return UserData(
        uid: doc.documentID,
        latitude:
            doc.data['latitude'].toDouble() ?? 0.0, // if 'latitude doesnt exist
